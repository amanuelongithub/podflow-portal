...
const ToastContainer = ({ toasts, removeToast }) => {
  ...
  const getToastClasses = (type) => {
    const baseClasses = 'rounded-lg shadow-lg p-4 mb-3 transform transition-all duration-300 ease-in-out max-w-sm';
        ...
        >
          <div className="flex items-start">
            ...
            <div className="ml-3 flex-1">
              <p className={`text-sm font-medium ${
                toast.type === TOAST_TYPES.SUCCESS ? 'text-green-800' :
                toast.type === TOAST_TYPES.ERROR ? 'text-red-800' :
                toast.type === TOAST_TYPES.WARNING ? 'text-yellow-800' :
                'text-blue-800'
              }`}>
                {toast.message}
              </p>
          ...
          <div className="mt-2">
            <div className={`h-1 rounded-full ${
              toast.type === TOAST_TYPES.SUCCESS ? 'bg-green-200' :
              toast.type === TOAST_TYPES.ERROR ? 'bg-red-200' :
              toast.type === TOAST_TYPES.WARNING ? 'bg-yellow-200' :
              'bg-blue-200'
            }`}>
              <div 
                className={`h-full rounded-full ${
                  toast.type === TOAST_TYPES.SUCCESS ? 'bg-green-500' :
                  toast.type === TOAST_TYPES.ERROR ? 'bg-red-500' :
                  toast.type === TOAST_TYPES.WARNING ? 'bg-yellow-500' :
                  'bg-blue-500'
                }`}
                style={{
